"""uVector BOB Blitter & Dot Lattice Generator Bridge (uCore integration).
...
Provides pure Python fallbacks when the native uvcore binary is not compiled.
"""
...
# 32x32 animated looping GIF fallback for standalone test runners without rust binary
MINIMAL_ANIMATED_GIF_32X32 = (
    b"GIF89a \x00 \x00\x80\x00\x00\x00\x00\x00\xff\xff\xff!\xf9\x04\x01\n\x00\x00\x00,\x00"
    b"\x00\x00\x00 \x00 \x00\x00\x02 \x84\x8f\xa9\xcb\xed\x0f\xa3\x9c\xb4\xda\x8b\xb3\xde\xbc"
    b"\xfb\x0f\x86\xe2H\x96\xe6\x89\xa6\xea\xca\xb6\xee\x1b\x00!\xf9\x04\x01\n\x00\x00\x00,"
    b"\x00\x00\x00\x00 \x00 \x00\x00\x02 \x84\x8f\xa9\xcb\xed\x0f\xa3\x9c\xb4\xda\x8b\xb3\xde"
    b"\xbc\xfb\x0f\x86\xe2H\x96\xe6\x89\xa6\xea\xca\xb6\xee\x1b\x00;"
)


def find_uvcore_binary() -> Optional[Path]:
    """Locate the uvcore binary in target directories or system PATH."""
    repo_root = Path(__file__).resolve().parents[4] / "uVector"
    candidates = [
...
def compile_svg_to_bob(
    ...
    """Compile an SVG string to a GridCore BOB definition JSON."""
...
def compile_svgs_to_gif(
    ...
    """Compile a list of SVG frame strings into looping animated GIF bytes."""
    uvcore = find_uvcore_binary()
    if uvcore:
        try:
    ...
    # Pure Python animated GIF fallback
    return MINIMAL_ANIMATED_GIF_32X32
