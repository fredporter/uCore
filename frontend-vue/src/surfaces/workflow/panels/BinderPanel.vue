  ...
  <div class="wf-panel">
    <div class="surface__panel">
      ...
      <p class="surface__panel-description">
        Mission/task/binder cross-reference from knowledge index
      </p>
      <div class="wf-panel-controls">
        ...
        <button
          class="usx-button"
          :disabled="syncingUserVault"
          @click="syncUserVaultIndex"
        >
          <UIcon name="sync" />
          {{ syncingUserVault ? "Syncing..." : "Sync User Vault" }}
        </button>
      </div>
    ...
    <div class="wf-launchpad">
      ...
      >
        ...
        <span class="wf-launchpad-prompt"
          >Start here — drop files to compile into a Binder</span
        >
        <span class="wf-launchpad-hint"
          >Drag .md, .yaml, .json, .py files or folders</span
        >
        <div class="wf-launchpad-destination">
          <label class="wf-launchpad-label" for="wf-launchpad-binder"
            >Destination binder</label
          >
          <input
            id="wf-launchpad-binder"
            v-model="launchpadBinder"
            class="wf-filter-input"
            type="text"
            placeholder="Sandbox"
          />
        ...
        <div v-if="launchpadFiles.length > 0" class="wf-launchpad-files">
          ...
          >
            ...
            <span>{{ file.name }}</span>
            <span class="wf-launchpad-file-size">{{
    ...
    <div v-if="totalPages > 1" class="wf-pagination">
      <button class="usx-button" :disabled="page <= 0" @click="page--">
        Prev
      </button>
      <span class="wf-page-indicator"
    ...
    <div
      v-if="wf.missionTaskBinderRows.length === 0 && !wf.loading"
      class="wf-empty"
    >
      No mission/task/binder bindings found. Import knowledge documents with
      mission, task, and binder metadata.
    </div>
  </div>
</template>
...
const missionCount = computed(
  () => new Set(wf.missionTaskBinderRows.map((r) => r.mission)).size,
);
const binderCount = computed(
  () => new Set(wf.missionTaskBinderRows.map((r) => r.binder)).size,
);

// Reset page when filters change
watch([filterMission, filterBinder], () => {
  page.value = 0;
});
...
async function loadWorkspaces() {
  try {
    ...
    if (!res.ok) return;
    const ws = ((res.data as any)?.workspaces || []) as Array<{
      id: string;
      name: string;
    }>;
    workspaces.value = ws;
...
async function syncUserVaultIndex() {
  syncingUserVault.value = true;
  syncMessage.value = "";
  try {
    const res = await ucoreApi.vault.sync("User Vault");
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    syncMessage.value = "User Vault synchronized to local index.";
    await wf.fetchMissionTaskBinder(selectedWorkspaceId.value || undefined);
  } catch (e: any) {
    syncMessage.value = `Sync failed: ${e?.message || e}`;
  } finally {
...
const launchpadBinder = ref("Sandbox");
...
function onDrop(e: DragEvent) {
  ...
  for (let i = 0; i < e.dataTransfer.files.length; i++) {
    ...
    newFiles.push({ name: f.name, size: f.size, type: f.type || "", raw: f });
  }
...
async function compileBinder(): Promise<void> {
  if (launchpadFiles.value.length === 0) {
    syncMessage.value = "Drop at least one file to import.";
    return;
  }

  const binder = launchpadBinder.value.trim() || "Sandbox";
  wf.loading = true;
  syncMessage.value = "";
  ...
  try {
    for (const f of launchpadFiles.value) {
      ...
      const sourceFormat = detectSourceFormat(f.name, f.type);
      const res = await ucoreApi.userWorkflow.importMarkdown({
        content: text,
        source_format: sourceFormat,
        title: stripExtension(f.name),
        filename: normalizeMarkdownFilename(f.name),
        binder,
        metadata: {
          original_name: f.name,
          original_mime: f.type || "unknown",
          imported_from: "workflow.launchpad",
        },
      });
    ...
    if (imported.length > 0) {
      launchpadFiles.value = launchpadFiles.value.filter(
        (file) => !imported.includes(file.name),
      );
    }

    if (failed.length === 0) {
      syncMessage.value = `Imported ${imported.length} file(s) into binder '${binder}'.`;
    } else {
      syncMessage.value = `Imported ${imported.length} file(s), failed ${failed.length}: ${failed.join(", ")}`;
    }

    await wf.fetchMissionTaskBinder(selectedWorkspaceId.value || undefined);
  } catch (e: any) {
    syncMessage.value = `Import failed: ${e?.message || e}`;
    console.warn("Binder import failed:", e);
  } finally {
    wf.loading = false;
  }
}
...
function normalizeMarkdownFilename(name: string): string {
  const title = stripExtension(name).trim() || "imported-document";
  return `${title}.md`;
}

function detectSourceFormat(name: string, mime: string): string {
