...
<template>
  <div class="edit-tab">
    <div class="edit-tab__toolbar">
      ...
      <button class="usx-btn" @click="undo" :disabled="historyIndex <= 0">Undo</button>
      <button class="usx-btn" @click="redo" :disabled="historyIndex >= history.length-1">Redo</button>
    </div>
    ...
    <div class="edit-tab__footer">
      <button class="usx-btn" @click="$emit('preview')">Preview</button>
      <button class="usx-btn usx-btn--primary" @click="$emit('save', { title: fmTitle, content: text, tags: (fmTags || '').split(',').map(t=>t.trim()).filter(Boolean) })">Save to Binder</button>
    </div>
  </div>
</template>
