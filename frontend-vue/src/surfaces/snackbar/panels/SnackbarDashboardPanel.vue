<template>
  <div class="server-dashboard" v-if="!srv.loading">
    <div class="server-dashboard-header">
      <div>
        <h2 class="usx-no-margin">Server Overview</h2>
    ...
    <div class="surface__panel">
      ...
      <div
        ...
        :key="svc.name"
        class="server-service-row"
      >
        <div class="server-service-info">
          <span
            class="server-service-dot"
            :class="'server-service-dot--' + svc.status"
          />
...
.server-stats-row {
  --server-stats-column-min: calc(var(--usx-touch-min) * 4.5);
  display: grid;
  grid-template-columns: repeat(
    auto-fit,
    minmax(min(100%, var(--server-stats-column-min)), 1fr)
  );
...
.server-stat-value--up {
  color: var(--usx-color-success);
}
.server-stat-value--degraded {
...
.server-service-dot--up {
  background: var(--usx-color-success);
}
.server-service-dot--degraded {
  background: var(--usx-color-warning);
}
.server-service-dot--down {
