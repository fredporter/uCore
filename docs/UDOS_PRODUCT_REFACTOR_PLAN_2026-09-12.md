...
- **Wizard Authority, Knowledge Layering & Federation Doctrine**:
  - The Wizard (system architect) maintains canonical base knowledge in external candidate/release source repositories (not via direct mutation of the installed runtime) using Antigravity IDE, `agy` CLI, and a defined set of Skills and Recipes running frontier models (Claude, Gemini, GPT via OpenRouter) plus local models (Ollama). Canonical updates are validated, packaged, and signed through uKnowledge packaging and release lanes before distribution as the read-only installed edition at `~/Public/global-knowledge/`. No in-app "Dev Mode" is needed inside uCore for dataset or standards development.
  - Sovereign users cannot corrupt or overwrite the canonical baseline; they customize and layer data *over top* using a cascading filter approach with explicit shadowing rules: Layer 2 Personal (`~/Vault/`) ➔ Layer 1 Shared (`~/Shared/`) ➔ Layer 0 Base Canon (`~/Public/global-knowledge/`). Same-relative-path documents shadow lower layers, metadata conflicts resolve in favor of the higher-precedence layer, while deletions in upper layers leave the underlying baseline canon intact.
  - Proprietary cloud roadmap introduces a user-initiated, opt-in contribution pipeline strictly for explicitly selected contribution packages (never sweeping private vaults). Submissions execute local privacy, licensing, and provenance pre-checks with optional opt-in linting, followed by Wizard review and approval before merging into canonical releases. Offline-first local operation remains completely functional without cloud participation.
