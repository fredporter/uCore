...
- **Wizard Authority, Knowledge Layering & Federation Doctrine**:
  - The Wizard (system architect) maintains canonical base knowledge (`~/Public/global-knowledge/`) and default datasets externally via Antigravity IDE, `agy` CLI, and a defined set of Skills and Recipes running frontier models (Claude, Gemini, GPT via OpenRouter) plus local models (Ollama). No in-app "Dev Mode" is needed inside uCore for dataset or standards development.
  ...
  - Proprietary cloud roadmap introduces a GitHub-style PR / Wikipedia-style submission pipeline where users bundle local vaults or proposed global knowledge diffs for automated AI linting and Wizard review/approval before merging into canonical releases.
